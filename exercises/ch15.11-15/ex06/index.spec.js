...
test.describe("simple todo app", () => {
  test.beforeEach(async ({ page }) => {
    await page.goto(
      "http://localhost:5500/exercises/ch15.11-15/ex04/index.html"
    );
  });
  ...
  test("todos are preserved after reload", async ({ page }) => {
    await addToDo(page, "質問表に質問を記載する");
    await addToDo(page, "練習問題を完了する");

    expect(await countToDos(page)).toBe(2);

    let todo1 = queryToDo(page, 0);
    await expect(todo1.getByText("練習問題を完了する")).toBeVisible();

    let todo2 = queryToDo(page, 1);
    await expect(todo2.getByText("質問表に質問を記載する")).toBeVisible();
    ...
    todo1 = queryToDo(page, 0);
    await expect(todo1.getByText("練習問題を完了する")).toBeVisible();

    todo2 = queryToDo(page, 1);
    await expect(todo2.getByText("質問表に質問を記載する")).toBeVisible();
  });
});
